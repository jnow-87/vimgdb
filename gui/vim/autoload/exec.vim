...
function! vimgdb#exec#init()
	...
	command! -nargs=0 -complete=custom,vimgdb#complete#lookup Nnexti call s:exec("nexti", <f-args>)
	command! -nargs=0 -complete=custom,vimgdb#complete#lookup Step call s:exec("step", <f-args>)
	command! -nargs=0 -complete=custom,vimgdb#complete#lookup Stepi call s:exec("stepi", <f-args>)
